#!/usr/bin/env node
/**
 * Parse the compressed 991 (911) workshop volumes into searchable procedure chunks.
 *
 *   node tools/manual/parse-991-workshop.mjs
 *
 * The 991 factory manual uses the same Mitchell/Porsche WM format as the 987
 * (`Service Manual: NN - …`, `WM <code> … > …`, functional-group codes), so this
 * reuses that chunker verbatim — it just reads public/manual/volumes-991.json +
 * the volume PDFs, tags generation=991 / source=workshop, prefixes ids so they
 * never collide with 981/987, and sets doc_id to the volume that owns each
 * absolute page.
 *
 * Output: data/manual-991.json (gitignored) → npm run db:import-991
 */
...
const manifestPath = path.join(ROOT, 'public/manual/volumes-991.json');
if (!fs.existsSync(manifestPath)) {
  console.error(`Missing ${manifestPath} — run compress-991-workshop.mjs first.`);
  process.exit(1);
}

const manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
const outFile = path.join(ROOT, 'data/manual-991.json');
const ID_PREFIX = '991-';
...
const NOISE_RE = [
  ...
  /^Service Manual:.*(Porsche|911|CARRERA|TARGA|TURBO)/,
  /^\s*(Porsche 911|911 Carrera|Back To Article)\s*$/i,
  /^Microsoft\s*$/i,
  /^Page \d+\s*$/i,
...
for (const vol of manifest.volumes) {
  ...
  const tmp = path.join(os.tmpdir(), `manual-991-${vol.id}-${Date.now()}.txt`);
...
console.log(`✓ ${deduped.length} chunks (${withWm} WM procedures) → ${path.relative(ROOT, outFile)}`);
console.log(`  total text: ${(deduped.reduce((n, r) => n + r.content.length, 0) / 1e6).toFixed(1)} MB`);
